  ...
  for (i=0; i<imx; i++) { //2048
     for(j=0; j<imy; j++) { //2048
         ...
         if(x<imx && y<imy){
           ...
           out[y*imx+x-(imx-1)].r = in[j*imx+i].r;
           out[y*imx+x-(imx-1)].i = in[j*imx+i].i;
         }
         else if (x<imx && y>=imy){
           out[(y-(imy-1))*imx+x].r = in[j*imx+i].r;
           out[(y-(imy-1))*imx+x].i = in[j*imx+i].i;
         }
         else if (x>=imx && y>=imy){
...
void mycircmask(unsigned char *in, unsigned char *out, int shift,bool right,bool down) { // mirror image is being generated here
  ...
  for (i=0; i<imx; i++) { //2048
     for(j=0; j<imy; j++) { //2048
         ...
         else if (x>=imx-1 && y<imy){
           out[y*imx+x-(imx-1)] = in[j*imx+i];
         }
         else if (x<imx && y>=imy-1){
           out[(y-(imy-1))*imx+x] = in[j*imx+i];
         }
         else if (x>=imx-1 && y>=imy-1){
...
//void findmidpoint(int width,int height,int shift,bool right,bool down,kiss_fft_cpx* imgk){
//  int i,j;
...
//  strcat(strx,"_");
//  if(shift){
//      int size_shift=(int)((ceil(log10(shift))+1)*sizeof(char));
//      char str_shift[size_shift];
//      sprintf(str_shift, "%d", shift);
//      strcat(strx,str_shift);
//      }
//  else{strcat(strx,"0");}
//  strcat(strx,"_");
//  if(right==1 && down==0) {strcat(strx,"r");}
//  else if(right==0 && down==1) {strcat(strx,"d");}
//  else if(right==0 && down==0){strcat(strx,"0");}
//  else {strcat(strx,"rd");}
//  
//  printf("key = %s ",strx);
//
...
//
//}
kiss_fft_cpx* calculateMidPointRect(int width,int height,int step,int numshift){
 int numx=width/step;
 int numy=height/step;
 
 kiss_fft_cpx *sum;
 sum	= (kiss_fft_cpx*) malloc( 1* 1* sizeof(kiss_fft_cpx) );
 sum->r=0.0;
 sum->i=0.0;
 FILE *fp;
 fp=fopen("test3.csv","w");
 fprintf(fp,"real,imag,index\n");
 for(int i=0;i<numy;i++){
   for(int j=0;j<numx;j++){
        sum->r=sum->r+midpointIntensity[(i)*numshift+(j)].r;
        sum->i=sum->i+midpointIntensity[(i)*numshift+(j)].i;
        printf("midpoint.r %lf midpoint.i %lf index %d \n",midpointIntensity[(i)*numshift+(j)].r,midpointIntensity[(i)*numshift+(j)].i,(i)*numshift+(j));
        fprintf(fp,"%lf,%lf,%d\n",midpointIntensity[(i)*numshift+(j)].r,midpointIntensity[(i)*numshift+(j)].i,(i)*numshift+(j));
   }
        printf("sum.r %lf sum.i %lf\n",sum->r,sum->i);
 }
 close(fp);
  return sum;
}

void LUT(kiss_fft_cpx *midpoint,int x,int y){

}
void make_LUT(int loopx,int loopy,int step,int numshift){
  ...
  for (int i=1;i<=loopx;i++){
    ...
    for(int j=1;j<=loopy;j++){
      ...
      strcat(separator,stry);
      strcat(strx,separator);
      //if (x==2048 && y==2048){rect=calculateMidPointRect(x,y,step,numshift);}
...
void findmidpoint(int index,kiss_fft_cpx *imgk,kiss_fft_cpx *midpointIntensity){
      int i,j;
      for(i=0;i<imx;i++){
      for(j=0;j<imy;j++){
        if(i==1024-1 && j==1024-1){ 
          midpointIntensity[index].r=imgk[j*imx+i].r;
          midpointIntensity[index].i=imgk[j*imx+i].i;
          printf("%f %d\n",midpointIntensity[index].r,index);
          break;
        }
      }
    }
...
int main(int argc, char	 **argv) {
  ...
  int numshift=imx/shift;
	...
	bmskout	= malloc(2048 * 2048);
	bmskout1	= malloc(2048 * 2048);
 ...
 // Config for output image
	...
	imgkout	= (kiss_fft_cpx*) malloc( dims[0] * dims[1] * sizeof(kiss_fft_cpx) );
	firsttile	= (kiss_fft_cpx*) malloc( dims[0] * dims[1] * sizeof(kiss_fft_cpx) );
	imgkout1	= (kiss_fft_cpx*) malloc( dims[0] * dims[1] * sizeof(kiss_fft_cpx) );
...
////////////////////////////////////////////////////////////////
  ...
  for( int l = 0; l<numrect; l++) {
    ...
    memset(imgkout, 0, dims[0] * dims[1] * sizeof(kiss_fft_cpx));
    ...
    for(int z=0;z<numshift;z++){
      int width;
      if(imx%shift>0 && z==numshift-1){
        mycircmask(bmsk,bmskout,z*shift+imx%shift,1,0);
        mycircShift(imgkin,imgkout,z*shift+imx%shift,1,0);
        width=key[0]+z*shift+imx%shift;
        }
      else{
        mycircmask(bmsk,bmskout,z*shift,1,0);
        mycircShift(imgkin,imgkout,z*shift,1,0);
        width=key[0]+z*shift;
      }
      int height=key[1];
      ...
      for(j=0;j<imy;j++){
        if(i==1024-1 && j==1024-1){ 
          midpointIntensity->r=midpointIntensity->r + imgkout[j*imx+i].r;
          midpointIntensity->i=midpointIntensity->i + imgkout[j*imx+i].i;
          printf("%f \n",midpointIntensity->r);
          break;
          }
        }
      ...
      //write_mask_file(bmskout, maskfile, 2048, 2048);//converts 2 image
      //findmidpoint(numshift+i,imgkout,midpointIntensity);
  ...
  //make_LUT(loop_x,loop_y,step,numshift);
