...
void myDevLayerDraw(unsigned char *imgpbo, int nx, int ny, int ox, int oy, int sc, rect *devdata, int lvlcnt, int c) {
  ...
  for( i = 0; i<lvlcnt; i++) {
    ...
    fprintf(stderr,"Rad rectangle (%d,%d) to (%d,%d)\n",x0,y0,x1,y1);
		printf("----------------------------------------------");
...
void myComputeImage(agsKernel *krn, int workx, int worky, float dose) { //kernel input is here
 ...
 // int nk = krn->nk;
  ...
  for(k=0; k<nk; k++) { // loopos nk times
    ...
    write_image_wo_th(imgpk,"Debug/imgk.png", 2048, 2048, dose);
...
int main(int argc, char	 **argv) {
 ...
 // Config for input mask
	...
	//printf("dims[0] = %d dims[1]= %d",dims[0],dims[1]);
 // Config for output image
  ...
  realPart = malloc(2048 * 2048 * sizeof(float));
  imgPart = malloc(2048 * 2048 * sizeof(float));
...
printf("\n\n**********After myMaskFloat***********\n\n");
 ...
 //
  ...
  write_image_wo_th(img, "Debug/imagwoth.png", 2048, 2048, dose);
